...
function M.parse(arg)
	...
	cmd:option('-gray', 	   false, 					    'grayscale rgb')
	...
	cmd:option('-loadHeight', 	   128, 						'loadSize[3]')
	cmd:option('-loadWidth', 	   171, 						'loadSize[4]')
	cmd:option('-sampleHeight',    112, 						'sampleSize[3]')
	cmd:option('-sampleWidth', 	   112, 						'sampleSize[4]')
    ...
    cmd:option('-normalize',       0,                       'normalize flow/edr activation or not')
	cmd:option('-perframemean',    false,                	'Whether the mean per frame should be subtracted')
	cmd:option('-minmax',          false,                	'Minmax normalization')
	...
	cmd:option('-crops10',         true,                	'Whether the test will be done on 10 crops or the center crop.')
	...
	cmd:option('-LRfile', 		   './LR/UCF101/flow_d5.lua','Path to file where opt.regimes is defined')
